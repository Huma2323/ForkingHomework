...
#the following codes work with selector gadget
# go to chrome to activate selector gadget
#select or deselect things on the chrome, then copy the path that appears in the box at the  bottom
#paste it where it says.temp and you will only see those items appear
...
parse_number(Temperatures) #This drops any non-numeric characters before or after the first number.
?parse_number
...
#organizing the data using a data frame
...
#to convert a dataframe to tibble
as_tibble(Weather)
...
datatables <- wikiurl%>%
  html_table(., fill = T) #fill=T is because we want all contents of the page

?html_table
datatables[[4]] -> dt   #same pages have several tables, this line indicated the table we want
dt
...
#when you insert these tables in r, there will be inconsistencies which you can fix using dplyr
#example 2a
wikiurl <- read_html(
  "https://evadoption.com/factors-contributing-to-ev-sales-market-share-by-us-state/")
datatables2 <- wikiurl%>%
  html_table(., fill = T)

#you can merge two tibbles 
datatables2[[1]] -> dt1
dt1
datatables2[[2]] -> dt2
dt2
df <- data.frame()
left_join(dt1, dt2, by = c("State" = "State")) 
